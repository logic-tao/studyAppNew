...
export class VideosPage {
  ...
  img_data = []
  sid:any
  constructor(@Inject('appService') private appService,public navParams: NavParams, public navCtrl: NavController,public appComponent:MyApp) {
    this.sid = this.navParams.data.sid;
  }
  ionViewDidLoad() {
    console.log('ionViewDidLoad VideoPage');
    console.log(this.navParams.data)
    this.searchlession(this.navParams.data.inpustring,this.navParams.data.sid)
  }
  searchlession(str,sid){
    let servedata={name:str,tokenId: this.appComponent.userinfo.tokenId}
    // sid	否	string	非必填，录播课id
    // name	是	string	录播课名称关键字
    this.appService.mysearchlession(servedata).then(
      res => {
        if(res.code==200){
